...
pub async fn post_login(
    ...
    let session_token_res = get_token(
        "s".to_owned() + session_id.to_string().as_str(),
        now,
        session_exp,
    );
...
pub async fn post_refresh(
    ...
    if let Some(st) = req.cookie("session_token") {
        session_token = st.value().to_string();
    } else {
        session_token = data.into_inner().session_token;
    }
    let now = Utc::now();
    let user_exp = now + Duration::minutes(15);
    ...
    if !claims.sub.starts_with("s") {
        return HttpResponse::BadRequest().body(format!("It's not session token, bruh"));
    }
    ...
    let session_res = db::user::get_session_lite(&pool, &session_id).await;
    ...
    if !is_valid {
        return HttpResponse::Unauthorized().body("Session ID is invalid :(");
    }
    ...
    HttpResponse::Accepted()
        .cookie(user_cookie)
        .json(RefreshRespond { user_token })
}
